...
impl Datetime {
    ...
    pub fn today(
        ...
        if time {
            let Some(datetime) = engine.world.today_with_time(offset) else {
                bail!(
                    "unable to get the current time of day";
                    hint: "set a fixed time source with the `--creation-timestamp` \
                           argument or the `SOURCE_DATE_EPOCH` environment variable";
                );
            };
            Ok(datetime)
        } else {
            let Some(datetime) = engine.world.today(offset) else {
                bail!("unable to get the current date");
            };
            Ok(datetime)
        }
    }
